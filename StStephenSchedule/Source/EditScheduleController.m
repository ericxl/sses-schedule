...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:[NSString stringWithFormat:@"For %@ Day Only",self.editingDayDisplayedName] style:UIAlertActionStyleDefault handler:^(UIAlertAction * action){
        NSInteger row = [indexPath row]+1;
...
-(void)setEditedDataFromPicker:(NSDictionary *)editedData {
    if (![editedData isEqual:_editedDataFromPicker]) {
        _editedDataFromPicker=editedData;
